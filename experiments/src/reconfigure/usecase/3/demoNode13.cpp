 ...
 * declaration
 */
bool demoNode13ClientCallback(reconfigure::demoNodeService::Request &req,
...
int main(int argc, char ** argv) {
    ENTER();
    std::string node_name = "demoNode13";
    ...
    ros::SCNServiceServer service = n.advertiseService(node_name, gCallbackService, demoNode13ClientCallback);

    // service called of demo node 11
    std::string testServiceClient13 = "demoNode11TestService";
    ros::SCNServiceClient testClient = n.serviceClient<reconfigure::demoNodeService>(node_name, testServiceClient13);
    ...
    while(ros::ok())
    {
        if (testClient.call(srv)) {
        	...
        	{
        		ROS_INFO("Called demoNode11TestService");
        	}
        	else
        	{
        		ROS_INFO("Called demoNode11TestService error");
        	}

        } else {
            ROS_ERROR("Failed to call demoNode11TestService");
        }
        ros::spinOnce();
 ...
 */
bool demoNode13ClientCallback(reconfigure::demoNodeService::Request &req, reconfigure::demoNodeService::Response &res) {
    ENTER();
    ...
    ROS_INFO("Demo Node 13 Enter safe mode!\n");
