 ...
 * declaration
 */
bool demoNode7ClientCallback(reconfigure::demoNodeService::Request &req,
        reconfigure::demoNodeService::Response &res);

void saveStateCb(uint8_t reconType) {
}

STATUS_T reconModeCb7(uint8_t reconType, uint8_t command) {
    return SCN_ST_OK;
}
/**
 * APIs
...
int main(int argc, char ** argv) {
    ENTER();
    std::string node_name = "demoNode7";
    //ros::init(argc, argv, node_name);
    ros::scnInit(argc, argv, node_name, 0, saveStateCb, reconModeCb7);
    ros::SCNNodeHandle n;
    ...
    ros::SCNServiceServer service = n.advertiseService(node_name, gCallbackService, demoNode7ClientCallback);

    // service called of demo node 6
    std::string testServiceClient7 = "demoNode6TestService";
    ros::SCNServiceClient testClient = n.serviceClient<reconfigure::demoNodeService>(node_name, testServiceClient7);
    ...
    while(ros::ok())
    {
        if (testClient.call(srv)) {
        	...
        	{
        		ROS_INFO("Called demoNode6TestService");
        	}
        	else
        	{
        		ROS_INFO("Called demoNode6TestService error");
        	}

        } else {
            ROS_ERROR("Failed to call demoNode6TestService");
        }
        ros::spinOnce();
 ...
 */
bool demoNode7ClientCallback(reconfigure::demoNodeService::Request &req, reconfigure::demoNodeService::Response &res) {
    ENTER();
    ...
    ROS_INFO("Demo Node 7 Enter safe mode!\n");
