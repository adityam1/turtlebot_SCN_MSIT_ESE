...
STATUS_T reconModeCb8(uint8_t reconType, uint8_t command) {
    return SCN_ST_OK;
}
/**
 * APIs
...
int main(int argc, char ** argv) {
    ENTER();
    std::string node_name = "demoNode8";
//    ros::init(argc, argv, node_name);
    ros::scnInit(argc, argv, node_name, 0, saveStateCb, reconModeCb8);
    ros::SCNNodeHandle n;
    ...
    ros::SCNServiceServer service = n.advertiseService(node_name, gCallbackService, demoNode8ClientCallback);

    // service provided for demo node 5
    std::string testService8 = "demoNode4TestService";
    ros::SCNServiceServer testService = n.advertiseService(node_name, testService8, demoNode8CallBack);
 ...
 */
bool demoNode8ClientCallback(reconfigure::demoNodeService::Request &req, reconfigure::demoNodeService::Response &res) {
    ENTER();
    ...
    ROS_INFO("Demo Node 8 Enter safe mode!\n");
    ROS_INFO("Currently, nothing to do in safe mode, will specify later!\n");

    //ROS_INFO("Demo Node 8 Leave safe mode!\n");
